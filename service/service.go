...
func (svc *Service) Start(ctx context.Context) chan []EpisodeStatusChange {
	svc.jobsQueue.Subscribe(ctx, queueEventCreateEpisodes, func(jobCtx context.Context, payload []byte) error {
		return svc.onCreateEpisodesQueueEvent(jobCtx, payload)
	})
	svc.jobsQueue.Subscribe(ctx, queueEventPollEpisodesStatus, func(jobCtx context.Context, payload []byte) error {
		return svc.onPollEpisodesQueueEvent(jobCtx, payload)
	})
	svc.jobsQueue.Subscribe(ctx, queueEventRegenerateFeed, func(jobCtx context.Context, payload []byte) error {
		return svc.onRegenerateFeedQueueEvent(jobCtx, payload)
	})
